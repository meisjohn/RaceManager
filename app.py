...
def save_data():
    data = {
        ...
        "races": [r.__dict__ for r in races],
...
def index():
    ...
    if request.method == "POST":
        ...
        try:
            patrol = int(request.form.get("patrol")) # Get patrol
            if 1 <= patrol <= len(patrol_names):
                ...
                error = f"Patrol must be between 1 and {len(patrol_names)}."  # Set error message
        except ValueError:
            error = "Invalid patrol input. Please enter a number."  # Set error message
...
def delete_participant(participant_id):
    ...
    # Renumber car numbers for the same patrol
    patrol = participant.patrol
    same_patrol_participants = sorted([p for p in participants 
                                       if p.patrol == patrol], 
                                      key=lambda p: p.car_number) # Sort by car number
    for i, p in enumerate(same_patrol_participants):
        p.car_number = i + 1
        p.car_name = f"{patrol_names.get(patrol, 'Unknown')[:1]}{p.car_number}"  # Update car name
...
def schedule_semi_final_races(patrol):
    ...
    # 1. Select Top Racers (same as before)
    first_round_races = [r for r in races if r.round == Rounds.FIRST and r.patrol == patrol]
    if not first_round_races: # If no races have been run yet, then we can't do semi-finals
        return

    all_racer_averages = {}
    for race in first_round_races:
    ...
    top_racers = [racer for racer, avg_time in sorted_racers[:NUM_LANES]]

    # 2. Create Race and Heats (using existing logic):
    race_groups = [top_racers]  # Create a single group of top racers
    assign_paired_lanes(race_groups, Rounds.SEMI, len(races) + 1) # Reuse existing function!
...
def schedule_final_races():
    ...
    for patrol in patrol_names:
        if patrol != "Exhibition" and semi_final_races_completed.get(patrol, False):  # Check if semi-finals are complete
            semi_final_races = [r for r in races if r.round == Rounds.SEMI and r.patrol == patrol]
            if semi_final_races: # Only if semi-final races exist for this patrol
                all_racer_averages = {}
                for race in semi_final_races:
                    race_averages = calculate_race_averages(race)
                    for racer, avg_time in race_averages.items():
                        if racer not in all_racer_averages:
                          all_racer_averages[racer] = []
                ...
                sorted_racers = sorted(overall_racer_averages.items(), key=lambda item: item[1])
                top_racer = next((racer for racer, avg_time in sorted_racers[:1]), None) # Get the top racer
                if top_racer:
                    top_racers.append(top_racer)
          ...
          if time is not None:
              try:
                  ...
                  if participant:
                      ...
                      participant.best_time = min(participant.times) if participant.times else 0
              except ValueError:
                  return "Invalid time input", 400
      calculate_averages()
      save_data() # Saving data after entering race times
      return redirect(url_for("schedule", patrol=race.patrol.value, round=race.round.value)) # Add round parameter
...
@app.route("/complete_initial/<int:patrol>")
...
@app.route("/schedule_semifinal/<int:patrol>")
...
@app.route("/schedule_final/<int:patrol>")
...
def schedule():
    ...
    return render_template("schedule.html", races=races, patrol_names=patrol_names,
                           selected_patrol=selected_patrol, selected_round=selected_round,
                           selected_round_name = selected_round_name, Rounds=Rounds, NUM_LANES=NUM_LANES,
                           initial_races_completed=initial_races_completed)
...
def get_top_racers_for_round(round_num, num_top_racers=NUM_LANES):
    round_races = [race for race in races if race.round == round_num]
    all_racer_averages = {}
    ...
    sorted_racers = sorted(overall_racer_averages.items(), key=lambda item: item[1])
    return [racer for racer, avg_time in sorted_racers[:num_top_racers]]

def get_top_time_race_number(racer):
    top_time = float('inf') # Initialize to infinity
    top_time_race_number = None

    for i, time in enumerate(racer.times):
        if time is not None and time < top_time: # Check for valid times and if this time is the best time
            top_time = time
            # Find the race number for this time
            race_number = next((race.race_number for race in races for heat in race.heats for lane, p in heat.lanes.items() if p == racer and i < len(heat.times) and heat.times[lane] == time), None)
            if race_number:
                top_time_race_number = race_number
    return top_time_race_number
...
def download_results():
    ...
    for p in participants:  # Assuming participants is your list of racers
        writer.writerow([p.first_name + " " + p.last_name, patrol_names.get(p.patrol), p.car_number, p.car_name, p.average_time, p.best_time, p.top_time_race_number])
